...
const ConfirmationModal = memo(({ isOpen, onCancel, onConfirm, title, description }: iConfirmationModalProps) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-[#000000aa] flex items-center justify-center z-50">
      <div className="bg-bg p-6 rounded-2xl shadow-2xl max-w-md w-full relative">
        {isLoading ? (
          ...
          <>
            <h2 className="text-2xl font-semibold text-txt text-center mb-4">
              {title}
            </h2>
            <p className="text-txt text-center mb-6">{description}</p>
